...
const { serviceHandler, getServiceCurrency } = require('./rechargeHandler');
...
// 人民币支付方式
Controllers.handleRechargeRequestForyuan = async (req, res, serviceInfo) => {
...
// 声望支付方式
Controllers.handleRechargeRequestForreputation = async (req, res, serviceInfo) => {
	const servicePrice = parseInt(serviceInfo.servicePrice, 10);
	const metaSettings = await meta.settings.get("recharge");
	const isOpen = metaSettings["close-vip-cost-entry"];
	const isClose = isOpen && JSON.parse(isOpen.toLowerCase());
	if (isClose !== false) {
		throw new Error("[[recharge:not-open-now]]");
	}
	// 检查用户的声望值是否充足
	const uid = String(req.uid);
	const currentReputation = await user.getUserField(uid, 'reputation');
	if (currentReputation < servicePrice) {
		return controllerHelpers.formatApiResponse(400, res, new Error(`[[recharge:not-enough-reputation, ${currentReputation}]]`));
	}

	const expireDay = parseInt(serviceInfo.serviceParams.split(',')[0], 10);
	if (!isFinite(expireDay)) {
        throw new Error("[[recharge:invalid-data]]");
    }

	const data = await serviceHandler[serviceInfo.serviceTypes](uid, expireDay);
	data.leftReputation = await user.incrementUserReputationBy(uid, -servicePrice);
	await events.log({
		type: 'reputation-cost-succeed',
		uid,
		ip: req.ip,
		expireDay,
		serviceTypes: serviceInfo.serviceTypes,
		servicePrice: servicePrice,
	});
	// 给用户字段增加标记
	controllerHelpers.formatApiResponse(200, res, data);
}
...
Controllers.handleRechargeRequest = async (req, res) => {
	...
	const costCurrency = getServiceCurrency(serviceInfo);
	const func = Controllers['handleRechargeRequestFor' + costCurrency];
	if (!func) {
		winston.error(`[error] recharge request error ${serviceInfo.serviceId}, ${serviceInfo.serviceTypes}`);
		return;
	}

	return func(req, res, serviceInfo);
}
		...
		if (status < 6) {
			...
			if (rechargeGroupName) {
				const [isGroupExist, isMember] = await Promise.all([
					...
					groups.isMember(rechargeData.client_order_uid, rechargeGroupName),
				]);
				if (isGroupExist && !isMember) {
					await groups.join([rechargeGroupName], rechargeData.client_order_uid);
				}
			}
			// 订单完成
